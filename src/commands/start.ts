...
const command: StartStopCommand = {
  ...
  details: "You must specify 1-7 other players to play with, and there can't \
...
must be used in the game's channel, or in DMs.",
  ...
  Game | undefined => {
    ...
    const maybePlayers: (User | undefined)[] = playerIDs.map(
      (id: string): User | undefined => m.client.users.cache.get(id));
